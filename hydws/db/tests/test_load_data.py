...
json_file_path_no_starttime = os.path.join(
    current_dir, 'data/borehole_file_no_section_times.json')
...
class LoadBoreholeTestCase(unittest.TestCase):
    ...
    def test_load_no_section_metadata(self):
        ...
        # Check that the section start and end times have been updated
        # to match the first and last hydraulic samples.
        self.assertEqual(bh0._sections[0].starttime,
                         datetime.datetime(2006, 12, 9, 23))
        self.assertEqual(bh0._sections[0].endtime,
                         datetime.datetime(2006, 12, 10, 10))

    def test_load_no_section_starttime(self):
        os.remove(db_filename)
        subprocess.run(
            ['hydws-db-init', '-o',
             db_sqlite_string],
        ...
        subprocess.run(
            ['hydws-data-import',
             db_sqlite_string, json_file_path_no_starttime],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
        session = Session()
        bh0 = session.query(orm.Borehole).\
